...
//------------------------------------------------------------------------------
// Notes:	(1) Reservoir switch selects according to index so as long
//			as iressw always has same options, this is ok.  If
//			one is removed or doesn't go in numerical order, that
//			will need to be looked at in this code.
//------------------------------------------------------------------------------
// History:
// 
// 01 Oct 1997	Catherine E.		Created initial version of class
//		Nutting-Lane, RTi
...
// 15 Aug 2001	SAM, RTi		Automatically select first item in the
//					list.
// 2002-09-16	SAM, RTi		Change the name of the daily time series
//					Vector.
//------------------------------------------------------------------------------
// 2003-06-09	J. Thomas Sapienza, RTi	Initial swing version from 
//					SMReservoirsWindow.
// 2003-06-11	JTS, RTi		Began using StateMod_DataSetComponent
//					and StateMod_Reservoir code.
...
// 2003-07-16	JTS, RTi		Added a constructor that allows a 
//					reservoir to be selected initially.
...
// 2003-08-03	SAM, RTi		* indexOf() moved from StateMod_Data to
//					  StateMod_Util.
//					* Force title parameter in constructor.
// 2003-08-18	SAM, RTi		Synchronize with recent changes in the
//					old GUI:
//					* Change labels to be consistent with
//					  data components.
//					* Change time series to JCheckBoxes and
//					  implement a combo-plot for data.
//					* Change actionPerformed() to use the
//					  objects, not action strings.
// 2003-08-22	JTS, RTi		Changed resDailyID to a Combo Box.
...
// 2003-08-29	SAM, RTi		Update because of changes in
//					StateMod_Reservoir.
// 2003-09-04	JTS, RTi		Slight changes to the daily data it
//					field and combo box.
// 2003-09-05	JTS, RTi		Class is now an item listener in 
//					order to enable/disable graph buttons
//					based on selected checkboxes.
// 2003-09-08	JTS, RTi		* Adjusted the layout to look better.
//					* Added checkTimeSeriesButtonsStates()
//					  to enable or disable the time series
...
//					* Blank cresdy values are now 
//					  defaulted to "0".
// 2004-10-28	SAM, RTi		Use new table model that only contains
//					reservoir station data.
...
// 2006-03-05	SAM, RTi		* Change "Capacity" to "Content" in the
//					  button label for reservoir curve data.
...
//ORIGINAL LINE: @SuppressWarnings("serial") public class StateMod_Reservoir_JFrame extends javax.swing.JFrame implements java.awt.event.ActionListener, java.awt.event.ItemListener, java.awt.event.KeyListener, java.awt.event.MouseListener, java.awt.event.WindowListener, RTi.Util.GUI.JWorksheet_SortListener
	public class StateMod_Reservoir_JFrame : JFrame, ActionListener, ItemListener, KeyListener, MouseListener, WindowListener, JWorksheet_SortListener
	{
	...
	/// GUI Buttons.
	/// </summary>
	private JButton __applyJButton = null, __areaCapacityContent = null, __cancelJButton = null, __climateFactors = null, __closeJButton = null, __helpJButton = null, __findNextRes = null, __ownerAccounts = null, __waterRights = null, __returnFlow_JButton = null, __showOnMap_JButton = null, __showOnNetwork_JButton = null;

	private SimpleJButton __graph_JButton = null, __summary_JButton = null, __table_JButton = null;

	private JCheckBox __ts_precipitation_monthly_JCheckBox = null, __ts_evaporation_monthly_JCheckBox = null, __ts_maxtarget_monthly_JCheckBox = null, __ts_mintarget_monthly_JCheckBox = null, __ts_maxtarget_daily_JCheckBox = null, __ts_maxtarget_est_daily_JCheckBox = null, __ts_mintarget_daily_JCheckBox = null, __ts_mintarget_est_daily_JCheckBox = null, __ts_content_monthly_JCheckBox = null, __ts_content_daily_JCheckBox = null, __ts_content_est_daily_JCheckBox = null;
	...
	/// GUI Textfields.
	/// </summary>
	private JTextField __deadStorageInRes = null, __maxReservoirContent = null, __maxReservoirRelease = null, __minReservoirContent = null, __reservoirLocation = null, __reservoirName = null, __reservoirStationID = null, __searchID = null, __searchName = null;
	...
	internal int __resDailyIDBaseSize = 4;

	/// <summary>
	/// Strings to be displayed on buttons.
	/// </summary>
	private const string __BUTTON_SHOW_ON_MAP = "Show on Map", __BUTTON_SHOW_ON_NETWORK = "Show on Network", __BUTTON_APPLY = "Apply", __BUTTON_CANCEL = "Cancel", __BUTTON_CLOSE = "Close", __BUTTON_HELP = "Help";
	...
	/// </summary>
	private IList<StateMod_Reservoir> __reservoirsVector = null;

	/// <summary>
	/// Constructor. </summary>
	/// <param name="dataset"> dataset containing the reservoir data </param>
	/// <param name="dataset_wm"> the dataset window manager or null if the data set windows are not being managed. </param>
	/// <param name="editable"> whether the form dat is editable or not </param>
	public StateMod_Reservoir_JFrame(StateMod_DataSet dataset, StateMod_DataSet_WindowManager dataset_wm, bool editable)
	{
		...
		__reservoirsVector = (IList<StateMod_Reservoir>)__reservoirComponent.getData();

		int size = __reservoirsVector.Count;
		for (int i = 0; i < size; i++)
		{
			StateMod_Reservoir r = (StateMod_Reservoir)__reservoirsVector[i];
			r.createBackup();
		}
	...
	/// <summary>
	/// Constructor. </summary>
	/// <param name="dataset"> the dataset containing the reservoir data. </param>
	/// <param name="dataset_wm"> the dataset window manager or null if the data set windows are not being managed. </param>
	/// <param name="reservoir"> the reservoir to select from the list. </param>
	/// <param name="editable"> whether the form dat is editable or not. </param>
	public StateMod_Reservoir_JFrame(StateMod_DataSet dataset, StateMod_DataSet_WindowManager dataset_wm, StateMod_Reservoir reservoir, bool editable)
	{
		StateMod_GUIUtil.setTitle(this, dataset, "Reservoirs", null);
		JGUIUtil.setIcon(this, JGUIUtil.getIconImage());
		__dataset = dataset;
	...
	public virtual void actionPerformed(ActionEvent e)
	{
		...
		try
		{
			...
			else if (source == __closeJButton)
			{
				...
				for (int i = 0; i < size; i++)
				{
					StateMod_Reservoir r = __reservoirsVector[i];
			...
			else if (source == __showOnMap_JButton)
			{
				...
				__dataset_wm.showOnMap(getSelectedReservoir(), "Res: " + getSelectedReservoir().getID() + " - " + getSelectedReservoir().getName(), new GRLimits(shape.xmin, shape.ymin, shape.xmax, shape.ymax), geoRecord.getLayer().getProjection());
			}
			else if (source == __showOnNetwork_JButton)
			{
				...
				{
					HydrologyNode node = networkEditor.getNetworkJComponent().findNode(getSelectedReservoir().getID(), false, false);
					if (node != null)
					{
						__dataset_wm.showOnNetwork(getSelectedReservoir(), "Res: " + getSelectedReservoir().getID() + " - " + getSelectedReservoir().getName(), new GRLimits(node.getX(),node.getY(),node.getX(),node.getY()));
					}
				}
			}
			// Time series buttons...
	...
	private void checkTimeSeriesButtonsStates()
	{
		...
		if ((__ts_precipitation_monthly_JCheckBox.isSelected() && __ts_precipitation_monthly_JCheckBox.isEnabled()) || (__ts_evaporation_monthly_JCheckBox.isSelected() && __ts_evaporation_monthly_JCheckBox.isEnabled()) || (__ts_content_monthly_JCheckBox.isSelected() && __ts_content_monthly_JCheckBox.isEnabled()) || (__ts_content_daily_JCheckBox.isSelected() && __ts_content_daily_JCheckBox.isEnabled()) || (__ts_content_est_daily_JCheckBox.isSelected() && __ts_content_est_daily_JCheckBox.isEnabled()) || (__ts_maxtarget_monthly_JCheckBox.isSelected() && __ts_maxtarget_monthly_JCheckBox.isEnabled()) || (__ts_mintarget_monthly_JCheckBox.isSelected() && __ts_mintarget_monthly_JCheckBox.isEnabled()) || (__ts_maxtarget_daily_JCheckBox.isSelected() && __ts_maxtarget_daily_JCheckBox.isEnabled()) || (__ts_mintarget_est_daily_JCheckBox.isSelected() && __ts_mintarget_est_daily_JCheckBox.isEnabled()) || (__ts_mintarget_daily_JCheckBox.isSelected() && __ts_mintarget_daily_JCheckBox.isEnabled()) || (__ts_mintarget_est_daily_JCheckBox.isSelected() && __ts_mintarget_est_daily_JCheckBox.isEnabled()))
		{
			enabled = true;
		}
	...
	private void displayTSViewJFrame(object o)
	{
		...
		int its = 0;
		TS ts = null;
		StateMod_Reservoir res = ((StateMod_Reservoir)__reservoirsVector[__currentReservoirIndex]);
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<RTi.TS.MonthTS> precip_tslist = (java.util.List<RTi.TS.MonthTS>)(__dataset.getComponentForComponentType(StateMod_DataSet.COMP_PRECIPITATION_TS_MONTHLY)).getData();
		IList<MonthTS> precip_tslist = (IList<MonthTS>)(__dataset.getComponentForComponentType(StateMod_DataSet.COMP_PRECIPITATION_TS_MONTHLY)).getData();
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<RTi.TS.MonthTS> evap_tslist = (java.util.List<RTi.TS.MonthTS>)(__dataset.getComponentForComponentType(StateMod_DataSet.COMP_EVAPORATION_TS_MONTHLY)).getData();
		IList<MonthTS> evap_tslist = (IList<MonthTS>)(__dataset.getComponentForComponentType(StateMod_DataSet.COMP_EVAPORATION_TS_MONTHLY)).getData();
		StateMod_ReservoirClimate clim = null;
		if (Message.isDebugOn)
		{
			Message.printDebug(1, "", "SAMX size evapts Vector = " + evap_tslist.Count);
		...
		if ((__ts_precipitation_monthly_JCheckBox.isSelected() && (res.getNumPrecipitationMonthTS(precip_tslist,true) > 0)) || (__ts_evaporation_monthly_JCheckBox.isSelected() && (res.getNumEvaporationMonthTS(evap_tslist,true) > 0)))
		{
			...
			int nclim = res.getClimates().Count;
			if (Message.isDebugOn)
			{
		...
		if ((__ts_content_monthly_JCheckBox.isSelected() && res.getContentMonthTS() != null) || (__ts_content_daily_JCheckBox.isSelected() && res.getContentDayTS() != null) || (__ts_maxtarget_monthly_JCheckBox.isSelected() && res.getMaxTargetMonthTS() != null) || (__ts_maxtarget_daily_JCheckBox.isSelected() && res.getMaxTargetDayTS() != null))
		{
...
//ORIGINAL LINE: protected void finalize() throws Throwable
	~StateMod_Reservoir_JFrame()
	{
		...
		__worksheet = null;
		__reservoirSwitch = null;
		__oneFillRuleAdmin = null;
		__dataset = null;
		__reservoirComponent = null;
		__reservoirsVector = null;
...
//		base.finalize();
	...
	{
		return __reservoirsVector[__currentReservoirIndex];
	}
	...
	private void rebuildResDailyID()
	{
		...
		IList<string> ids = StateMod_Util.createIdentifierListFromStateModData(__reservoirsVector, false, null);

		for (int i = 0; i < ids.Count; i++)
		{
			string currID = (string)ids[i];
			if (!currID.Trim().Equals(id))
			{
				__resDailyID.add(currID + " - Daily TS pattern of other reservoir used with monthly TS from " + "this reservoir.");
			}
		}
	}
	...
	private void saveInformation(int record)
	{
		...
		StateMod_Reservoir res = (StateMod_Reservoir)__reservoirsVector[record];
	...
	/// <summary>
	/// Selects a row from the left table, scrolls to that row, and displays its information. </summary>
	/// <param name="id"> the id of the row to select </param>
	...
	/// the GUI (ie, if there is no value, it is defaulted to 0). </summary>
	/// <param name="r"> the StateMod_Reservoir to default the original cresdy. </param>
	/// <param name="cresdy"> the cresdy to default to. </param>
	...
	public virtual void setupGUI(int index)
	{
		...
		JGUIUtil.addComponent(mainJPanel, jsw, 0, 0, 3, 1, 1, 1, 0, 0, 0, 20, GridBagConstraints.BOTH, GridBagConstraints.NORTHWEST);
		JGUIUtil.addComponent(mainJPanel, right, 3, 0, 1, 18, 0, 1, 0, 0, 0, 10, GridBagConstraints.NONE, GridBagConstraints.NORTHEAST);
		...
		y = 0;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Reservoir Station ID:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __reservoirStationID, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__reservoirStationID.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Name:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __reservoirName, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__reservoirName.addActionListener(this);

		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("River Node ID:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __reservoirLocation, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__reservoirLocation.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Daily Data ID:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __resDailyID, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Storage Switch:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __reservoirSwitch, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);

		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Minimum Content (AF):"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __minReservoirContent, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__minReservoirContent.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Maximum Content (AF):"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __maxReservoirContent, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__maxReservoirContent.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel,new JLabel("Maximum Release (CFS):"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __maxReservoirRelease, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__maxReservoirRelease.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("Dead Storage (AF):"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __deadStorageInRes, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__deadStorageInRes.addActionListener(this);
		y++;
		JGUIUtil.addComponent(param_JPanel, new JLabel("One Fill Rule Admin:"), 0, y, 1, 1, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(param_JPanel, __oneFillRuleAdmin, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(right, param_JPanel, 0, 0, 2, 11, 1, 1, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.NORTHWEST);
		JGUIUtil.addComponent(right, tsPanel, 0, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.NORTHWEST);
		JGUIUtil.addComponent(right, subformsPanel, 1, y, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.NORTHWEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_precipitation_monthly_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_evaporation_monthly_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_content_monthly_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_content_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_content_est_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_maxtarget_monthly_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_mintarget_monthly_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_maxtarget_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_maxtarget_est_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_mintarget_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, __ts_mintarget_est_daily_JCheckBox, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(tsPanel, tsb_JPanel, 0, y4++, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);

		int y3 = 0;
		y3++;
		JGUIUtil.addComponent(subformsPanel, __areaCapacityContent, 0, y3, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);
		y3++;
		JGUIUtil.addComponent(subformsPanel, __climateFactors, 0, y3, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);
		y3++;
		JGUIUtil.addComponent(subformsPanel, __ownerAccounts, 0, y3, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);
		y3++;
		JGUIUtil.addComponent(subformsPanel, __waterRights, 0, y3, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);
		y3++;
		JGUIUtil.addComponent(subformsPanel, __returnFlow_JButton, 0, y3, 1, 1, 1, 1, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.CENTER);
		...
		JGUIUtil.addComponent(mainJPanel, searchPanel, 0, y, 2, 1, 0, 0, 10, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		int y2 = 0;
		JGUIUtil.addComponent(searchPanel, __searchIDJRadioButton, 0, y2, 1, 1, 0, 0, 5, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(searchPanel, __searchID, 1, y2, 2, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__searchID.addActionListener(this);
		y2++;
		JGUIUtil.addComponent(searchPanel, __searchNameJRadioButton, 0, y2, 1, 1, 0, 0, 5, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(searchPanel, __searchName, 1, y2, 2, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__searchName.addActionListener(this);
		y2++;
		JGUIUtil.addComponent(searchPanel, __findNextRes, 1, y2, 2, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		...
		*/
		...
		JGUIUtil.addComponent(bottomJPanel, p5, 0, 0, 8, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		...
		if (__dataset_wm != null)
		{
			__dataset_wm.setWindowOpen(StateMod_DataSet_WindowManager.WINDOW_RESERVOIR, this);
		}
		pack();
