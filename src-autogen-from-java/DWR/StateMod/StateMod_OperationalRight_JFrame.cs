...
//ORIGINAL LINE: @SuppressWarnings("serial") public class StateMod_OperationalRight_JFrame extends javax.swing.JFrame implements java.awt.event.ActionListener, java.awt.event.KeyListener, java.awt.event.MouseListener, java.awt.event.WindowListener, RTi.Util.GUI.JWorksheet_SortListener
	public class StateMod_OperationalRight_JFrame : JFrame, ActionListener, KeyListener, MouseListener, WindowListener, JWorksheet_SortListener
	{

	/// <summary>
	/// Button labels.
	/// </summary>
	private readonly string __BUTTON_SHOW_ON_MAP = "Show on Map", __BUTTON_SHOW_ON_NETWORK = "Show on Network", __BUTTON_APPLY = "Apply", __BUTTON_ADD = "Add", __BUTTON_CANCEL = "Cancel", __BUTTON_CLOSE = "Close", __BUTTON_DELETE = "Delete", __BUTTON_HELP = "Help";
	...
	/// GUI buttons.
	/// </summary>
	private JButton __findNextOpr, __helpJButton, __closeJButton, __cancelJButton, __applyJButton, __showOnMap_JButton = null, __showOnNetwork_JButton = null;
	...
	/// </summary>
	private JPanel __attributes_JPanel, __destination_JPanel, __source_JPanel, __rioGrande_JPanel, __sanJuan_JPanel, __monthlySwitch_JPanel, __interveningStructuresWithoutLoss_JPanel, __interveningStructuresWithLoss_JPanel, __comments_JPanel, __textEditor_JPanel, __monthlyOprMax_JPanel, __monthlyOprEff_JPanel, __associatedOperatingRule_JPanel;
	...
	/// GUI text fields.
	/// </summary>
	private JTextField __oprAdminNumber_JTextField, __oprName_JTextField, __oprID_JTextField, __sanJuanSjmina_JTextField, __sanJuanSjrela_JTextField, __conveyanceLoss_JTextField, __limits_JTextField, __firstYear_JTextField, __lastYear_JTextField, __qdebt_JTextField, __qdebtx_JTextField, __rioGrandeCoefficient_JTextField, __rioGrandeBasin_JTextField, __rioGrandeBasinYield_JTextField, __rioGrandeDrain_JTextField, __rioGrandeDrainYield_JTextField;

	/// <summary>
	/// Text area for editing the operational right, when right type is not supported.
	/// </summary>
	private JTextArea __comments_JTextArea, __textEditor_JTextArea;
	...
	/// </summary>
	private SimpleJComboBox __opr8_JComboBox, __destination_JComboBox, __destinationAccount_JComboBox, __source1_JComboBox, __source2_JComboBox, __source3_JComboBox, __source4_JComboBox, __source5_JComboBox, __sourceAccount1_JComboBox, __sourceAccount2_JComboBox, __sourceAccount3_JComboBox, __sourceAccount4_JComboBox, __sourceAccount5_JComboBox, __ruleTypeSwitch_JComboBox, __oprSwitch_JComboBox, __associatedPlan_JComboBox, __diversionType_JComboBox, __associatedOperatingRule_JComboBox, __rioGrandeIndexGage_JComboBox;
	...
	/// </summary>
	private SimpleJComboBox[] __interveningStructuresWithoutLoss_JComboBox = new SimpleJComboBox[StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES];

	/// <summary>
	/// Combobox array for holding intervening structures (with loss).
	/// </summary>
	private SimpleJComboBox[] __interveningStructuresWithLoss_JComboBox = new SimpleJComboBox[StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES];

	/// <summary>
	/// Combobox array for holding intervening structures (with loss) type.
	/// </summary>
	private SimpleJComboBox[] __interveningStructuresWithLossType_JComboBox = new SimpleJComboBox[StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES];
	...
	/// </summary>
	private IList<StateMod_OperationalRight> __operationalRights;

	private int __currentItyopr = -1;

	/// <summary>
	/// Constructor. </summary>
	/// <param name="dataset"> the StateMod_DataSet object that has the data. </param>
	...
	public StateMod_OperationalRight_JFrame(StateMod_DataSet dataset, StateMod_DataSet_WindowManager dataset_wm, bool editable)
	{
		...
		__operationalRightsComponent = __dataset.getComponentForComponentType(StateMod_DataSet.COMP_OPERATION_RIGHTS);
		__operationalRights = (IList<StateMod_OperationalRight>)__operationalRightsComponent.getData();
		int size = __operationalRights.Count;
		StateMod_OperationalRight o = null;
		for (int i = 0; i < size; i++)
		{
			o = __operationalRights[i];
			o.createBackup();
		}
	...
	/// <param name="dataset"> the StateMod_DataSet object that has the data. </param>
	/// <param name="dataset_wm"> the dataset window manager or null if the data set windows are not being managed. </param>
	/// <param name="operationalRight"> the operational right to select from the list </param>
	/// <param name="editable"> whether the data are editable or not. </param>
	public StateMod_OperationalRight_JFrame(StateMod_DataSet dataset, StateMod_DataSet_WindowManager dataset_wm, StateMod_OperationalRight operationalRight, bool editable)
	{
		StateMod_GUIUtil.setTitle(this, dataset, "Operational Rights", null);
		__currentOpRightsIndex = -1;
		JGUIUtil.setIcon(this, JGUIUtil.getIconImage());
		...
		__dataset_wm = dataset_wm;
		__operationalRightsComponent = __dataset.getComponentForComponentType(StateMod_DataSet.COMP_OPERATION_RIGHTS);
		__operationalRights = (System.Collections.IList)__operationalRightsComponent.getData();
		int size = __operationalRights.Count;
		StateMod_OperationalRight o = null;
		for (int i = 0; i < size; i++)
		{
	...
	public virtual void actionPerformed(ActionEvent e)
	{
		...
		else if (action.Equals(__BUTTON_CLOSE))
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				{
					Message.printStatus(2, "", "Operational right [" + i + "] \"" + o.getID() + "\" has changed - accepting changes (discarding backup).");
					changed = true;
				}
				o.acceptChanges();
			}
		...
		else if (source == __showOnMap_JButton)
		{
			...
			__dataset_wm.showOnMap(opr, "OpRight: " + opr.getID() + " - " + opr.getName(), limits, limitsProjection);
		}
		else if (source == __showOnNetwork_JButton)
		{
			...
			if (networkEditor != null)
			{
				...
				if (smdataDest != null)
				{
					HydrologyNode nodeDest = networkEditor.getNetworkJComponent().findNode(smdataDest.getID(), false, false);
					if (nodeDest != null)
					{
				...
				{
					HydrologyNode nodeSource1 = networkEditor.getNetworkJComponent().findNode(smdataSource1.getID(), false, false);
					if (nodeSource1 != null)
					{
						if (limits == null)
						{
				...
				{
					HydrologyNode nodeSource2 = networkEditor.getNetworkJComponent().findNode(smdataSource2.getID(), false, false);
					if (nodeSource2 != null)
					{
						if (limits == null)
						{
				...
				__dataset_wm.showOnNetwork(opr, "OpRight: " + getSelectedOperationalRight().getID() + " - " + getSelectedOperationalRight().getName(), limits);
			}
		}
		// Operational right data...
		else if (source == __destination_JComboBox)
		{
			populateOperationalRightDestinationAccount(getSelectedOperationalRight(), __destination_JComboBox.getSelected(),__editable);
		...
		else if (source == __source1_JComboBox)
		{
			...
			StateMod_OperationalRight opr = __operationalRights[__currentOpRightsIndex];
	...
	{
		IList<string> errors = new List<object>();
		int errorCount = 0;
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		...
		if (smdataSource1 is HasGeoRecord)
		{
		...
		if (smdataSource2 is HasGeoRecord)
		{
...
//ORIGINAL LINE: protected void finalize() throws Throwable
	~StateMod_OperationalRight_JFrame()
	{
...
//		base.finalize();
	...
	private string getDataString(SimpleJComboBox comboBox)
	{
		...
		if (!string.ReferenceEquals(selected, null))
		{
			if (selected.IndexOf(" - ", StringComparison.Ordinal) >= 0)
			{
				// Typical string.  Return the first token before the space
				dataString = selected.Split(" ", true)[0].Trim();
			}
			else
			{
		...
		if (dataString.Equals(""))
		{
			if (comboBox.isEditable())
			{
				...
				if ((!string.ReferenceEquals(fieldText, null)) && !fieldText.Equals(""))
				{
	...
	/// <summary>
	/// Trims a string in the format "XXX - YYY" by removing everything after 'XXX'. </summary>
	...
	{
		return __operationalRights[__currentOpRightsIndex];
	}
	...
	/// <summary>
	/// Fills in the display information for the specified operational right. </summary>
	/// <param name="opr"> the operational right to use to populate the fields for right information. </param>
	...
	/// </summary>
	private void populateOperationalRightAssociatedOperatingRule(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
		...
		else
		{
			...
			{
				StateMod_OperationalRight_Metadata_SourceOrDestinationType[] operatingRuleType = new StateMod_OperationalRight_Metadata_SourceOrDestinationType[] {StateMod_OperationalRight_Metadata_SourceOrDestinationType.OPERATIONAL_RIGHT};
				StateMod_DataSet dataset = getDataSet();
				IList<string> operatinalRightIDStrings = new List<object>();
				// Always allow blank...
				operatinalRightIDStrings.Add("");
				((IList<string>)operatinalRightIDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, operatingRuleType, true));
				...
				try
				{
					JGUIUtil.selectTokenMatches(__associatedOperatingRule_JComboBox, true, " ", 0, 0, cx, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
	...
	/// </summary>
	private void populateOperationalRightAttributes(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
			((TitledBorder)__attributes_JPanel.getBorder()).setTitle("Primary attributes (right is NOT fully editable - use text editor)");
		...
		try
		{
			JGUIUtil.selectTokenMatches(__ruleTypeSwitch_JComboBox, true, "-", 0, 0, ("" + ityopr), null, true); // Trim tokens before comparing
		}
		catch (Exception)
		{
		...
		IList<string> oprSwitchChoices = new List<object>();
		...
		try
		{
			JGUIUtil.selectTokenMatches(__oprSwitch_JComboBox, true, " ", 0, 0, ("" + ioprsw), null, true); // Trim tokens before comparing
		}
		catch (Exception)
		{
		...
		if (metadata.getRightTypeUsesAssociatedPlan())
		{
			...
			StateMod_OperationalRight_Metadata_AssociatedPlanAllowedType[] allowedPlanTypes = metadata.getAssociatedPlanAllowedTypes();
			StateMod_DataSet dataset = getDataSet();
			IList<string> planIDStrings = new List<object>();
			// Always allow blank...
			planIDStrings.Add("");
			...
			((IList<string>)planIDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, allowedPlanTypes, true));
			__associatedPlan_JComboBox.setData(planIDStrings);
			try
			{
				JGUIUtil.selectTokenMatches(__associatedPlan_JComboBox, true, " ", 0, 0, creuse, null, true); // Trim tokens before comparing
			}
			catch (Exception)
			{
				// If here the data includes a year so add and select
		...
		if (metadata.getRightTypeUsesDiversionType())
		{
			...
			IList<string> diversionTypeStrings = new List<object>();
	...
	/// Populate operational right for the text editor panel.
	/// </summary>
	private void populateOperationalRightComments(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		// Always enabled
		// Set up the text editor text
		StringBuilder b = new StringBuilder();
		string nl = "\n"; // As per java doc this should be used in memory
		foreach (string s in opr.getCommentsBeforeData())
		{
			b.Append(s + nl);
		}
	...
	/// Populate operational right for the text editor panel.
	/// </summary>
	private void populateOperationalRightDestination(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
		...
		else
		{
			...
			{
				StateMod_OperationalRight_Metadata_SourceOrDestinationType[] allowedDestinationTypes = metadata.getDestinationTypes();
				StateMod_DataSet dataset = getDataSet();
				IList<string> destIDStrings = new List<object>();
				// Always allow blank...
				destIDStrings.Add("");
				((IList<string>)destIDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, allowedDestinationTypes, true));
				...
				try
				{
					JGUIUtil.selectTokenMatches(__destination_JComboBox, true, " ", 0, 0, ciopde, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
					// If here the data includes destination not in the data set so add to the list
	...
	/// This method is only going to be called when a destination is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	/// <param name="destinationSelection"> the value in the destination field </param>
	private void populateOperationalRightDestinationAccount(StateMod_OperationalRight opr, string destinationSelection, bool editable)
	{
		string destinationID = getFirstToken(destinationSelection);
		// The only time that accounts are other than 1 is when a reservoir
		__destinationAccount_JComboBox.removeAllItems();
		if (destinationSelection.IndexOf("(" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.RESERVOIR + ")", StringComparison.Ordinal) > 0)
		{
			// Get the accounts for the reservoir
			StateMod_DataSet dataset = getDataSet();
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<StateMod_Reservoir> reservoirList = (java.util.List<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
			IList<StateMod_Reservoir> reservoirList = (IList<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
			int pos = StateMod_Util.IndexOf(reservoirList,destinationID);
			IList<string> accountChoices = new List<string>();
			if (pos >= 0)
			{
				StateMod_Reservoir res = reservoirList[pos];
				IList<string> positiveAccounts = StateMod_Util.createIdentifierListFromStateModData(res.getAccounts(), true, null);
				((IList<string>)accountChoices).AddRange(positiveAccounts);
				if (opr.getMetadata().getRightTypeUsesNegativeDestinationAccounts())
				{
					// Add the accounts with negatives...
					foreach (string positiveAccount in positiveAccounts)
					{
						accountChoices.Add("-" + positiveAccount + " - fill up to this account based on ratio");
					}
				}
				...
				try
				{
					JGUIUtil.selectTokenMatches(__destinationAccount_JComboBox, true, " ", 0, 0, iopdes, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
	...
	/// This method is only going to be called when the destination account is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	private void populateOperationalRightDestinationAccountSpecial(StateMod_OperationalRight opr, bool editable)
	{
		__destinationAccount_JComboBox.removeAllItems();
		IList<string> specialChoices = opr.getMetadata().getDestinationAccountSpecialChoices();
		__destinationAccount_JComboBox.setData(specialChoices);
		// Now select
		string ciopde = opr.getCiopde();
		try
		{
			JGUIUtil.selectTokenMatches(__destinationAccount_JComboBox, true, " ", 0, 0, ciopde, null, true); // Trim tokens before comparing
		}
		catch (Exception)
		{
			// If here the data includes account not in the data set so add to the list
	...
	/// </summary>
	private void populateOperationalRightInterveningStructuresWithLoss(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
		...
		else
		{
			...
			{
				oprLossDouble = Convert.ToDouble(oprLoss);
			}
			if (metadata.getRightTypeUsesInterveningStructuresWithLoss(oprLossDouble))
			{
				 ...
				 */
				StateMod_OperationalRight_Metadata_SourceOrDestinationType[] structureTypes = new StateMod_OperationalRight_Metadata_SourceOrDestinationType[] {StateMod_OperationalRight_Metadata_SourceOrDestinationType.DIVERSION, StateMod_OperationalRight_Metadata_SourceOrDestinationType.OTHER};
				for (int i = 0; i < StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES; i++)
				{
					IList<string> structureIDStrings = new List<string>();
					// Always allow blank...
					structureIDStrings.Add("");
					((IList<string>)structureIDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, structureTypes, true));
					...
					else
					{
						try
						{
							JGUIUtil.selectTokenMatches(__interveningStructuresWithLoss_JComboBox[i], true, " ", 0, 0, intern, null, true); // Trim tokens before comparing
						}
						catch (Exception)
						{
							// If here the data includes destination not in the data set so add to the list
					...
					// Type...
					IList<string> typeStrings = new List<string>();
					...
					else
					{
						try
						{
							JGUIUtil.selectTokenMatches(__interveningStructuresWithLossType_JComboBox[i], true, " ", 0, 0, internT, null, true); // Trim tokens before comparing
						}
						catch (Exception)
						{
							// If here the data includes destination not in the data set so add to the list
	...
	/// Populate operational right for the intervening structures (without loss) data panel.
	/// </summary>
	private void populateOperationalRightInterveningStructuresWithoutLoss(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
				...
				*/
				...
				for (int i = 0; i < StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES; i++)
				{
					...
					else
					{
						try
						{
							JGUIUtil.selectTokenMatches(__interveningStructuresWithoutLoss_JComboBox[i], true, " ", 0, 0, intern, null, true); // Trim tokens before comparing
						}
						catch (Exception)
						{
							// If here the data includes destination not in the data set so add to the list
	...
	/// </summary>
	private void populateOperationalRightMonthlyOprEff(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
	...
	/// Populate operational right for the monthly operating limits data panel.
	/// </summary>
	private void populateOperationalRightMonthlyOprMax(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
		...
		else
		{
			...
			((TitledBorder)__monthlyOprMax_JPanel.getBorder()).setTitle(metadata.getMonthlyLimitsTitle());
			...
			{
				oprLimit = Convert.ToDouble(oprLimitString);
			}
	...
	/// </summary>
	private void populateOperationalRightMonthSwitch(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
			...
			if (metadata.getRightTypeUsesMonthlySwitch())
			{
				...
				for (int i = 0; i < 12; i++)
				{
					...
					try
					{
						...
						{
							JGUIUtil.selectTokenMatches(__monthSwitch_JComboBox[i], true, " ", 0, 0, "" + opr.getImonsw(i), null, true); // Trim tokens before comparing
						}
					}
					catch (Exception)
					{
	...
	/// </summary>
	private void populateOperationalRightRioGrande(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		int ityopr = opr.getItyopr();
		...
		else
		{
			...
			{
				// Not used
				IList<string> gageList = new List<string>();
				gageList.Add("");
			...
			else if (ityopr == 18)
			{
				...
				StateMod_OperationalRight_Metadata_SourceOrDestinationType[] allowedSourceTypes = new StateMod_OperationalRight_Metadata_SourceOrDestinationType[] {StateMod_OperationalRight_Metadata_SourceOrDestinationType.STREAM_GAGE};
				StateMod_DataSet dataset = getDataSet();
				IList<string> indexGageIDStrings = new List<string>();
				// Now add identifier choices...
				((IList<string>)indexGageIDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, allowedSourceTypes, true));
				...
				try
				{
					JGUIUtil.selectTokenMatches(__rioGrandeIndexGage_JComboBox, true, " ", 0, 0, ciopso3, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
	...
	/// </summary>
	private void populateOperationalRightSanJuan(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		int ityopr = opr.getItyopr();
	...
	/// </summary>
	private void populateOperationalRightSource(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
		...
		{
			bool rightTypeUsesSource1 = metadata.getRightTypeUsesSource1();
			bool rightTypeUsesSource2 = metadata.getRightTypeUsesSource2();
			bool rightTypeUsesSpecialSourceAccount1 = metadata.getRightTypeUsesSpecialSourceAccount1();
			bool rightTypeUsesSpecialSourceAccount2 = metadata.getRightTypeUsesSpecialSourceAccount2();
			if (rightTypeUsesSource1 || rightTypeUsesSource2 || rightTypeUsesSpecialSourceAccount1 || rightTypeUsesSpecialSourceAccount2)
			{
				...
				if (rightTypeUsesSource1)
				{
					...
					StateMod_OperationalRight_Metadata_SourceOrDestinationType[] allowedSourceTypes = metadata.getSource1Types();
					StateMod_DataSet dataset = getDataSet();
					IList<string> source1IDStrings = new List<string>();
					// Always allow blank...
					source1IDStrings.Add("");
					// Now add identifier choices...
					((IList<string>)source1IDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, allowedSourceTypes, true));
					__source1_JComboBox.setEnabled(true);
					...
					try
					{
						JGUIUtil.selectTokenMatches(__source1_JComboBox, true, " ", 0, 0, ciopso1, null, true); // Trim tokens before comparing
					}
					catch (Exception)
					{
				...
				if (rightTypeUsesSource2)
				{
					...
					if (source1Selection.IndexOf("(" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.PLAN_RECHARGE + ")", StringComparison.Ordinal) > 0)
					{
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<StateMod_Reservoir> reservoirList = (java.util.List<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
						IList<StateMod_Reservoir> reservoirList = (IList<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
						IList<string> reservoirChoices = StateMod_Util.createIdentifierListFromStateModData(reservoirList, true,null);
						...
						try
						{
							JGUIUtil.selectTokenMatches(__sourceAccount2_JComboBox, true, " ", 0, 0, ciopso2, null, true); // Trim tokens before comparing
						}
						catch (Exception)
						{
							// If here the data includes account not in the data set so add to the list
					...
					else
					{
						...
						string ciopso2 = opr.getCiopso2();
						StateMod_OperationalRight_Metadata_SourceOrDestinationType[] allowedSourceTypes = metadata.getSource2Types();
						StateMod_DataSet dataset = getDataSet();
						IList<string> source2IDStrings = new List<string>();
						// Always allow blank...
						source2IDStrings.Add("");
						// Add special choices...
						if (metadata.getRightTypeUsesSpecialSource2())
						{
							((IList<string>)source2IDStrings).AddRange(metadata.getSource2SpecialChoices());
						}
						((IList<string>)source2IDStrings).AddRange(StateMod_Util.createIdentifierList(dataset, allowedSourceTypes, true));
						__source2_JComboBox.setEnabled(true);
						__source2_JComboBox.setEditable(false); // Force user to select
						...
						try
						{
							JGUIUtil.selectTokenMatches(__source2_JComboBox, true, "-", 0, 0, ciopso2, null, true); // Trim tokens before comparing
						}
						catch (Exception)
						{
	...
	/// This method is only going to be called when source 1 is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	/// <param name="editable"> whether the data should be editable </param>
	private void populateOperationalRightSourceAccount1(StateMod_OperationalRight opr, bool editable)
	{
		string source1Selection = __source1_JComboBox.getSelected();
		...
		if (source1Selection.IndexOf("(" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.RESERVOIR + ")", StringComparison.Ordinal) > 0)
		{
			// Get the accounts for the reservoir
			StateMod_DataSet dataset = getDataSet();
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<StateMod_Reservoir> reservoirList = (java.util.List<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
			...
			if (pos >= 0)
			{
				...
				{
					((IList<string>)accountChoices).AddRange(metadata.getSourceAccount1SpecialChoices());
				}
				StateMod_Reservoir res = reservoirList[pos];
				// Add identifiers...
				((IList<string>)accountChoices).AddRange(StateMod_Util.createIdentifierListFromStateModData(res.getAccounts(), true, null));
				__sourceAccount1_JComboBox.setData(accountChoices);
				// Now select
				string iopsou1 = opr.getIopsou1();
				try
				{
					JGUIUtil.selectTokenMatches(__sourceAccount1_JComboBox, true, " ", 0, 0, iopsou1, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
					// If here the data includes account not in the data set so add to the list
	...
	/// This method is only going to be called when source 1 is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	private void populateOperationalRightSourceAccount1Special(StateMod_OperationalRight opr, bool editable)
	{
		__sourceAccount1_JComboBox.removeAllItems();
		IList<string> specialChoices = opr.getMetadata().getSourceAccount1SpecialChoices();
	...
	/// This method is only going to be called when source 2 is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	/// <param name="source2Selection"> the value in the source 1 field </param>
	private void populateOperationalRightSourceAccount2(StateMod_OperationalRight opr, bool editable)
	{
		...
		StateMod_OperationalRight_Metadata metadata = opr.getMetadata();
		if (source2Selection.IndexOf("(" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.RESERVOIR + ")", StringComparison.Ordinal) > 0)
		{
			// Get the accounts for the reservoir
			StateMod_DataSet dataset = getDataSet();
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") java.util.List<StateMod_Reservoir> reservoirList = (java.util.List<StateMod_Reservoir>)dataset.getComponentForComponentType(StateMod_DataSet.COMP_RESERVOIR_STATIONS).getData();
			...
			IList<string> accountChoices = new List<object>();
			if (pos >= 0)
			{
				StateMod_Reservoir res = reservoirList[pos];
				((IList<string>)accountChoices).AddRange(StateMod_Util.createIdentifierListFromStateModData(res.getAccounts(), true, null));
				...
				try
				{
					JGUIUtil.selectTokenMatches(__sourceAccount2_JComboBox, true, " ", 0, 0, iopsou2, null, true); // Trim tokens before comparing
				}
				catch (Exception)
				{
					// If here the data includes account not in the data set so add to the list
	...
	/// This method is only going to be called when source 2 is actually used. </summary>
	/// <param name="opr"> the current operational right </param>
	private void populateOperationalRightSourceAccount2Special(StateMod_OperationalRight opr, bool editable)
	{
		__sourceAccount2_JComboBox.removeAllItems();
		IList<string> specialChoices = opr.getMetadata().getSourceAccount2SpecialChoices();
	...
	/// Populate operational right for the text editor panel.
	/// </summary>
	private void populateOperationalRightTextEditor(StateMod_OperationalRight opr, StateMod_OperationalRight_Metadata metadata, bool useTextEditor, bool editable)
	{
		if (useTextEditor)
		{
			// Set up the text editor text
			StringBuilder b = new StringBuilder();
			string nl = "\n"; // As per Java doc, this should be used in memory
			foreach (string s in opr.getRightStrings())
			{
				b.Append(s + nl);
			}
	...
	private void saveInformation(int record)
	{
		...
		StateMod_OperationalRight opr = __operationalRights[record];
		...
		if (useTextEditor)
		{
			IList<string> rightStringList = new List<string>();
	...
	/// <param name="index"> the index of operational right to be initially selected </param>
	private void setupGUI(int index)
	{
		...
		JGUIUtil.addComponent(leftJPanel, setupGUI_OperationalRightList(routine), 0, 0, 1, 5, 1, 1, GridBagConstraints.BOTH, GridBagConstraints.NORTHWEST);

		// Search control under the main list...

		JGUIUtil.addComponent(leftJPanel, setupGUI_SearchPanel(routine), 0, 5, 1, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Operational right panels for general and optional parameters...

		int yRight = 0;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightAttributes(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Destination on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightDestination(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Sources on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightSource(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Monthly switches on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightMonthSwitch(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Monthly operational limits on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightMonthlyOprMax(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Monthly efficiencies on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightMonthlyOprEff(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Associated operating rule on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightAssociatedOperatingRule(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Intervening structures (without loss) on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightInterveningStructuresWithoutLoss(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Intervening structures (with loss) on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightInterveningStructuresWithLoss(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Rio Grande on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightRioGrande(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// San Juan on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightSanJuan(routine), 0, yRight, 12, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);

		// Comments for right, on the right...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightComments(routine), 0, yRight, 20, 1, 1.0, 1.0, GridBagConstraints.BOTH, GridBagConstraints.WEST);

		// Text editor on the right (make it wider than the other panels so everything is
		// not sized to it...

		++yRight;
		JGUIUtil.addComponent(rightJPanel, setupGUI_OperationalRightTextEditor(routine), 0, yRight, 20, 5, 1.0, 1.0, GridBagConstraints.BOTH, GridBagConstraints.WEST);

		// Add a blank panel to fill space, in particular when panels are disabled

		yRight += 5; // Height of the text editor
		JGUIUtil.addComponent(rightJPanel, new JPanel(), 0, yRight, 20, 5, 1.0, 1.0, GridBagConstraints.BOTH, GridBagConstraints.WEST);
		...
		*/
		...
		if (__dataset_wm != null)
		{
			__dataset_wm.setWindowOpen(StateMod_DataSet_WindowManager.WINDOW_OPERATIONAL_RIGHT, this);
		}
	...
	private JPanel setupGUI_Buttons(string routine)
	{
		...
		__helpJButton.setToolTipText("Show the operatingal rights documentation, including the decision tree for selecting the appropriate right");
	...
	private JPanel setupGUI_BottomPanel(string routine, GridBagLayout gb)
	{
		...
		JGUIUtil.addComponent(bottomJPanel, setupGUI_Buttons(routine), 0, 0, 8, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
	...
	private JPanel setupGUI_OperationalRightAssociatedOperatingRule(string routine)
	{
		...
		int y = 0;
		JGUIUtil.addComponent(__associatedOperatingRule_JPanel, new JLabel("Operating rule:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__associatedOperatingRule_JComboBox = new SimpleJComboBox(false); // Text field not editable
		__associatedOperatingRule_JComboBox.setPrototypeDisplayValue(" 123456789012 - (Operational Right) 123456789012345678901234 ");
		__associatedOperatingRule_JComboBox.setSelectionFailureFallback("~ - Unknown", 0);
		JGUIUtil.addComponent(__associatedOperatingRule_JPanel, __associatedOperatingRule_JComboBox, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		return __associatedOperatingRule_JPanel;
	}

	/// <summary>
	/// Setup the GUI components for the operational rights list. </summary>
	...
	private JPanel setupGUI_OperationalRightAttributes(string routine)
	{
		...
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Operational right ID:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __oprID_JTextField, 1, y, 1, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		__oprName_JTextField = new JTextField(24);
		__oprName_JTextField.setEditable(false);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Operational right name:"), 2, y, 1, 1, 0, 0, 0, 1, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __oprName_JTextField, 3, y, 1, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		y++;
		__ruleTypeSwitch_JComboBox = new SimpleJComboBox(false); // Text field not editable
		IList<StateMod_OperationalRight_Metadata> metadataList = StateMod_OperationalRight_Metadata.getAllMetadata();
		// OK to add these here because the list will not change with data selections
		for (int i = 0; i < metadataList.Count; i++)
		{
			__ruleTypeSwitch_JComboBox.add("" + metadataList[i].getRightTypeNumber() + " - " + metadataList[i].getRightTypeName());
		}
		__ruleTypeSwitch_JComboBox.setEnabled(false);
		__ruleTypeSwitch_JComboBox.setEditable(false);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Operational right type:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __ruleTypeSwitch_JComboBox, 1, y, width, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__ruleTypeSwitch_JComboBox.addActionListener(this);
		y++;
		__oprAdminNumber_JTextField = new JTextField(12);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Administration number:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __oprAdminNumber_JTextField, 1, y, 1, 1, 1, 0, 1, 1, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("On/off switch:"), 2, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __oprSwitch_JComboBox, 3, y, 1, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		y++;
		__associatedPlan_JComboBox = new SimpleJComboBox(false); // Text field not editable
		__associatedPlan_JComboBox.setPrototypeDisplayValue(" 123456789012 (Plan (Out of Priority Div. or Storage)) - 123456789012345678901234 ");
		__associatedPlan_JComboBox.setEditable(false);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Associated plan data:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __associatedPlan_JComboBox, 1, y, width, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		y++;
		...
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Diversion type:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __diversionType_JComboBox, 1, y, width, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		y++;
		__conveyanceLoss_JTextField = new JTextField(24);
		__conveyanceLoss_JTextField.setToolTipText("Blank will default to 0");
		__conveyanceLoss_JTextField.setEditable(false);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Conveyance loss (%):"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __conveyanceLoss_JTextField, 1, y, width, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		y++;
		__limits_JTextField = new JTextField(24);
		__limits_JTextField.setToolTipText("Blank will default to 0");
		__limits_JTextField.setEditable(false);
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Limits:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __limits_JTextField, 1, y, width, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		y++;
		...
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("First year of operation:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __firstYear_JTextField, 1, y, 1, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		...
		JGUIUtil.addComponent(__attributes_JPanel, new JLabel("Last year of operation:"), 2, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__attributes_JPanel, __lastYear_JTextField, 3, y, 1, 1, 1, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		return __attributes_JPanel;
	}

	/// <summary>
	/// Setup comments panel. </summary>
	/// <param name="routine"> </param>
	private JPanel setupGUI_OperationalRightComments(string routine)
	{
		__comments_JPanel = new JPanel();
		__comments_JPanel.setBorder(BorderFactory.createTitledBorder("Comments (will be output above operational right data, with # at start of line)"));
		GridBagLayout gb = new GridBagLayout();
		...
		JGUIUtil.addComponent(__comments_JPanel, new JScrollPane(__comments_JTextArea), 0, 0, 6, 1, 1, 1, 1, 1, 1, 1, GridBagConstraints.BOTH, GridBagConstraints.CENTER);
	...
	private JPanel setupGUI_OperationalRightDestination(string routine)
	{
		...
		int y = 0;
		JGUIUtil.addComponent(__destination_JPanel, new JLabel("Destination:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__destination_JComboBox = new SimpleJComboBox(false); // Text field not editable
		__destination_JComboBox.setPrototypeDisplayValue(" 123456789012 - (Instream Flow) 123456789012345678901234 ");
		__destination_JComboBox.setSelectionFailureFallback("~ - Unknown", 0);
		JGUIUtil.addComponent(__destination_JPanel, __destination_JComboBox, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__destination_JPanel, new JLabel("Account:"), 2, y, 1, 1, 0, 0, 0, 4, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		...
		JGUIUtil.addComponent(__destination_JPanel, __destinationAccount_JComboBox, 3, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__destination_JComboBox.addActionListener(this);
		return __destination_JPanel;
	}

	/// <summary>
	/// Set up the intervening structures (with loss) panel. </summary>
	/// <param name="routine"> for intervening structures </param>
	/// <returns> the panel for intervening structures </returns>
	private JPanel setupGUI_OperationalRightInterveningStructuresWithLoss(string routine)
	{
		...
		__interveningStructuresWithLoss_JPanel.setBorder(BorderFactory.createTitledBorder("Intervening structures with loss (specify up to " + StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES + ")"));
		GridBagLayout gb = new GridBagLayout();
		...
		for (int i = 0; i < nInterveningStructues; i++)
		{
			...
			__interveningStructuresWithLoss_JComboBox[i].setPrototypeDisplayValue(" 123456789012 - (Instream Flow) 123456789012345678901234 ");
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, new JLabel("" + (i + 1) + " ID:"), 0, i, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, __interveningStructuresWithLoss_JComboBox[i], 1, i, 1, 1, 0, 0, 1, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
			// Loss (percent)...
			__interveningStructuresWithLossPercent_JTextField[i] = new JTextField(10);
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, new JLabel("Loss %:"), 2, i, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, __interveningStructuresWithLossPercent_JTextField[i], 3, i, 1, 1, 1, 0, 0, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
			// Type...
			...
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, new JLabel("Type:"), 4, i, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
			JGUIUtil.addComponent(__interveningStructuresWithLoss_JPanel, __interveningStructuresWithLossType_JComboBox[i], 5, i, 1, 1, 0, 0, 1, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		}
		return __interveningStructuresWithLoss_JPanel;
	}

	/// <summary>
	/// Set up the intervening structures (without loss) panel. </summary>
	/// <param name="routine"> for intervening structures </param>
	/// <returns> the panel for intervening structures </returns>
	private JPanel setupGUI_OperationalRightInterveningStructuresWithoutLoss(string routine)
	{
		...
		__interveningStructuresWithoutLoss_JPanel.setBorder(BorderFactory.createTitledBorder("Intervening structures without loss (specify up to " + StateMod_OperationalRight_Metadata.MAXIMUM_INTERVENING_STRUCTURES + ")"));
		GridBagLayout gb = new GridBagLayout();
		...
		for (int i = 0; i < nInterveningStructues; i++)
		{
			...
			__interveningStructuresWithoutLoss_JComboBox[i].setPrototypeDisplayValue(" 123456789012 - (Instream Flow) 123456789012345678901234 ");
			JGUIUtil.addComponent(__interveningStructuresWithoutLoss_JPanel, new JLabel("" + (i + 1)), ((i / nRows) * 2), (i - (i / nRows) * nRows), 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
			JGUIUtil.addComponent(__interveningStructuresWithoutLoss_JPanel, __interveningStructuresWithoutLoss_JComboBox[i], ((i / nRows) * 2 + 1), (i - (i / nRows) * nRows), 1, 1, 0, 0, 1, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		}
	...
	private JScrollWorksheet setupGUI_OperationalRightList(string routine)
	{
		...
		try
		{
			StateMod_OperationalRight_TableModel tmo = new StateMod_OperationalRight_TableModel(__operationalRights, __editable);
	...
	/// <summary>
	/// Set up the monthly efficiency panel. </summary>
	/// <param name="routine"> for logging </param>
	/// <returns> the panel for monthly efficiencies </returns>
	private JPanel setupGUI_OperationalRightMonthlyOprEff(string routine)
	{
		__monthlyOprEff_JPanel = new JPanel();
		__monthlyOprEff_JPanel.setBorder(BorderFactory.createTitledBorder("T&C consumptive use factors (%)"));
		GridBagLayout gb = new GridBagLayout();
		...
		for (int i = 0; i < 12; i++)
		{
			JGUIUtil.addComponent(__monthlyOprEff_JPanel, new JLabel(TimeUtil.monthAbbreviation(months[i])), i - (i / 6) * 6, (i / 6) * 2, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
			__monthlyOprEff_JTextField[i] = new JTextField(6);
			JGUIUtil.addComponent(__monthlyOprEff_JPanel, __monthlyOprEff_JTextField[i], i - (i / 6) * 6, ((i / 6) * 2 + 1), 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		}
		return __monthlyOprEff_JPanel;
	}

	/// <summary>
	/// Set up the monthly operating limits panel. </summary>
	/// <param name="routine"> for logging </param>
	/// <returns> the panel for monthly operating limits </returns>
	private JPanel setupGUI_OperationalRightMonthlyOprMax(string routine)
	{
		__monthlyOprMax_JPanel = new JPanel();
		__monthlyOprMax_JPanel.setBorder(BorderFactory.createTitledBorder("Monthly and annual operating limits (ACFT)"));
		GridBagLayout gb = new GridBagLayout();
		...
		for (int i = 0; i < 12; i++)
		{
			JGUIUtil.addComponent(__monthlyOprMax_JPanel, new JLabel(TimeUtil.monthAbbreviation(months[i])), i - (i / 6) * 6, (i / 6) * 2, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
			__monthlyOprMax_JTextField[i] = new JTextField(6);
			JGUIUtil.addComponent(__monthlyOprMax_JPanel, __monthlyOprMax_JTextField[i], i - (i / 6) * 6, ((i / 6) * 2 + 1), 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		}
		// Add the annual value at the end.
		JGUIUtil.addComponent(__monthlyOprMax_JPanel, new JLabel("Annual"), 6, 2, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		__monthlyOprMax_JTextField[12] = new JTextField(6);
		JGUIUtil.addComponent(__monthlyOprMax_JPanel, __monthlyOprMax_JTextField[12], 6, 3, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		return __monthlyOprMax_JPanel;
	}

	/// <summary>
	/// Set up the monthly switch panel. </summary>
	/// <param name="routine"> for logging </param>
	/// <returns> the panel for monthly switches </returns>
	private JPanel setupGUI_OperationalRightMonthSwitch(string routine)
	{
		__monthlySwitch_JPanel = new JPanel();
		__monthlySwitch_JPanel.setBorder(BorderFactory.createTitledBorder("Monthly on/off switch (leave all blank for default of all on; otherwise, specify every value)"));
		GridBagLayout gb = new GridBagLayout();
		...
		for (int i = 0; i < 12; i++)
		{
			...
			JGUIUtil.addComponent(__monthlySwitch_JPanel, new JLabel(TimeUtil.monthAbbreviation(months[i])), i - (i / 6) * 6, (i / 6) * 2, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
			JGUIUtil.addComponent(__monthlySwitch_JPanel, __monthSwitch_JComboBox[i], i - (i / 6) * 6, ((i / 6) * 2 + 1), 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		}
	...
	private JPanel setupGUI_OperationalRightRioGrande(string routine)
	{
		...
		int x3 = 2;
		int x4 = 3;
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Year for adjustment:"), x1, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__qdebt_JTextField = new JTextField(20);
		__qdebt_JTextField.setToolTipText("Year when annual obligation calculation includes " + "an adjustment for the cumulative surplus storage.");
		JGUIUtil.addComponent(__rioGrande_JPanel, __qdebt_JTextField, x2, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Initial Surplus/Shortage (ACFT):"), x3, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__qdebtx_JTextField = new JTextField(6);
		JGUIUtil.addComponent(__rioGrande_JPanel, __qdebtx_JTextField, x4, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		++y;
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Index gage:"), x1, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeIndexGage_JComboBox = new SimpleJComboBox(false); // Text field not editable
		__rioGrandeIndexGage_JComboBox.setPrototypeDisplayValue(" 123456789012 - (Stream Gage) 123456789012345678901234 ");
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeIndexGage_JComboBox, x2, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Coefficient:"), x3, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeCoefficient_JTextField = new JTextField(6);
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeCoefficient_JTextField, x4, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		++y;
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Basin:"), x1, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeBasin_JTextField = new JTextField(20);
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeBasin_JTextField, x2, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Basin yield (ACFT/year):"), x3, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeBasinYield_JTextField = new JTextField(6);
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeBasinYield_JTextField, x4, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		++y;
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Drain:"), x1, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeDrain_JTextField = new JTextField(20);
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeDrain_JTextField, x2, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__rioGrande_JPanel, new JLabel("Drain yield (ACFT/year):"), x3, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__rioGrandeDrainYield_JTextField = new JTextField(6);
		JGUIUtil.addComponent(__rioGrande_JPanel, __rioGrandeDrainYield_JTextField, x4, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);

		return __rioGrande_JPanel;
	}

	/// <summary>
	/// Setup the panel used for San Juan recovery operational right. </summary>
	/// <param name="routine">
	/// @return </param>
	private JPanel setupGUI_OperationalRightSanJuan(string routine)
	{
		...
		int y = 0;
		JGUIUtil.addComponent(__sanJuan_JPanel, new JLabel("Minimum available water (CFS):"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__sanJuanSjmina_JTextField = new JTextField(10);
		JGUIUtil.addComponent(__sanJuan_JPanel, __sanJuanSjmina_JTextField, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		++y;
		JGUIUtil.addComponent(__sanJuan_JPanel, new JLabel("Average release (ACFT/year):"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		__sanJuanSjrela_JTextField = new JTextField(10);
		JGUIUtil.addComponent(__sanJuan_JPanel, __sanJuanSjrela_JTextField, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
	...
	private JPanel setupGUI_OperationalRightSource(string routine)
	{
		...
		__source_JPanel.setBorder(BorderFactory.createTitledBorder("Source(s) - note that Source 2 is used for special options for some right types"));

		__source1_JComboBox = new SimpleJComboBox(false); // Text field not editable
		__source1_JComboBox.setPrototypeDisplayValue(" 123456789012 - (" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.PLAN_OUT_OF_PRIORITY + ") 123456789012345678901234 ");
		...
		__source2_JComboBox.setPrototypeDisplayValue(" 123456789012 - (" + StateMod_OperationalRight_Metadata_SourceOrDestinationType.PLAN_OUT_OF_PRIORITY + ") 123456789012345678901234 ");
		...
		__sourceAccount2_JComboBox.setPrototypeDisplayValue(" 1 - Monthly diversion limit is in direct diversion demand ");
		...
		int y = 0;
		JGUIUtil.addComponent(__source_JPanel, new JLabel("Source 1:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__source_JPanel, __source1_JComboBox, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__source_JPanel, new JLabel("Account:"), 2, y, 1, 1, 0, 0, 0, 4, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__source_JPanel, __sourceAccount1_JComboBox, 3, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);

		y++;
		JGUIUtil.addComponent(__source_JPanel, new JLabel("Source 2:"), 0, y, 1, 1, 0, 0, 0, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__source_JPanel, __source2_JComboBox, 1, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(__source_JPanel, new JLabel("Account:"), 2, y, 1, 1, 0, 0, 0, 4, 0, 0, GridBagConstraints.NONE, GridBagConstraints.EAST);
		JGUIUtil.addComponent(__source_JPanel, __sourceAccount2_JComboBox, 3, y, 1, 1, 0, 0, 1, 0, 0, 1, GridBagConstraints.NONE, GridBagConstraints.WEST);
	...
	private JPanel setupGUI_SearchPanel(string routine)
	{
		...
		JGUIUtil.addComponent(psearch, __searchIDJRadioButton, 0, y, 1, 1, 0, 0, 1, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		__searchID = new JTextField(10);
		JGUIUtil.addComponent(psearch, __searchID, 1, y, 1, 1, 1.0, 0.0, 1, 0, 0, 0, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		...
		__searchName.setEditable(false);
		JGUIUtil.addComponent(psearch, __searchNameJRadioButton, 0, y, 1, 1, 0, 0, 1, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
		JGUIUtil.addComponent(psearch, __searchName, 1, y, 1, 1, 1.0, 1.0, 1, 0, 0, 0, GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		__searchName.addActionListener(this);
		y++;
		__findNextOpr = new JButton("Find Next");
		JGUIUtil.addComponent(psearch, __findNextOpr, 0, y, 4, 1, 0, 0, 5, 0, 0, 0, GridBagConstraints.NONE, GridBagConstraints.WEST);
	...
	/// <summary>
	/// Setup text editor panel, used when the GUI does not know how to handle the details of an operational right. </summary>
	/// <param name="routine"> </param>
	private JPanel setupGUI_OperationalRightTextEditor(string routine)
	{
		...
		__textEditor_JPanel.setBorder(BorderFactory.createTitledBorder("Operational right text (for types where detailed editing has not been implemented)"));
		GridBagLayout gb = new GridBagLayout();
		...
		JGUIUtil.addComponent(combined_JPanel, ref_JTextArea, 0, 0, 6, 1, 1, 1, 1, 1, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		JGUIUtil.addComponent(combined_JPanel, __textEditor_JTextArea, 0, 1, 6, 1, 1, 1, 1, 1, 1, 1, GridBagConstraints.BOTH, GridBagConstraints.CENTER);
		JGUIUtil.addComponent(__textEditor_JPanel, new JScrollPane(combined_JPanel), 0, 0, 6, 1, 1, 1, 1, 1, 1, 1, GridBagConstraints.BOTH, GridBagConstraints.CENTER);
	...
	public virtual void windowClosing(WindowEvent e)
	{
		...
		for (int i = 0; i < size; i++)
		{
			o = (StateMod_OperationalRight) __operationalRights[i];
			if (!changed && o.changed())
			{
				changed = true;
			}
