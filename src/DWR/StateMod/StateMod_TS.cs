...
namespace DWR.StateMod
{
    ...
    public class StateMod_TS
    {
        ...
        public static IList<TS> readTimeSeriesList(string fname, DateTime date1, DateTime date2, string units, bool read_data)
        {
            ...
            @in = IOUtil.getInputStream(full_fname);
        ...
        private static IList<TS> readTimeSeriesList(TS req_ts, StreamReader @in, string fullFilename, int fileInterval, DateTime reqDate1, DateTime reqDate2, string reqUnits, bool readData)
        {
            ...
            DateTime date = null;
            //TODO @jurentie 04/04/2019 - removed for now
            ...
            {
                Message.printWarning(1, routine, "Requested file interval is invalid.");
        ...
        private static void writeTimeSeriesList(TextWriter @out, IList<TS> tslist, DateTime date1, DateTime date2, YearType outputYearType, double MissingDV, int req_precision, bool print_genesis)
        {
            ...
            @out.WriteLine(cmnt);
            @out.WriteLine(cmnt + " StateMod time series");
            @out.WriteLine(cmnt + " ********************");
            @out.WriteLine(cmnt);
            if (outputYearType == YearType.WATER)
            {
                @out.WriteLine(cmnt + " Years Shown = Water Years (Oct to Sep)");
            }
            else if (outputYearType == YearType.NOV_TO_OCT)
            {
                @out.WriteLine(cmnt + " Years Shown = Irrigation Years (Nov to Oct)");
            }
            else
            {
                // if ( output_format.equalsIgnoreCase ("CYR" ))
                @out.WriteLine(cmnt + " Years Shown = Calendar Years");
            }
            @out.WriteLine(cmnt + " The period of record for each time series may vary");
            @out.WriteLine(cmnt + " because of the original input and data processing steps.");
            @out.WriteLine(cmnt);

            // Print each time series id, description, and type...

            @out.WriteLine(cmnt + "     TS ID                    Type" + "   Source   Units  Period of Record    Location    Description");
            ...
            for (int i = 0; i < nseries; i++)
            {
                ...
                if (print_genesis)
                {
                    ...
                    if (genesis != null)
                    {
                        ...
                        if (size > 0)
                        {
                            @out.WriteLine(cmnt + "      Time series creation history:");
                            for (int igen = 0; igen < size; igen++)
                            {
                                @out.WriteLine(cmnt + "      " + (string)genesis[igen]);
                            }
                        }
                    }
                }
            ...
            if (req_interval_base == TimeInterval.MONTH)
            {
                if (outputYearType == YearType.WATER)
                {
                    @out.WriteLine(cmnt + " Yr ID            Oct     Nov     Dec     Jan" + "     Feb     Mar     Apr     May     Jun     Jul" + "     Aug     Sep     " + year_title);
                }
                else if (outputYearType == YearType.NOV_TO_OCT)
                {
                    @out.WriteLine(cmnt + " Yr ID            Nov     Dec     Jan     Feb" + "     Mar     Apr     May     Jun     Jul     Aug" + "     Sep     Oct     " + year_title);
                }
                else
                {
                    @out.WriteLine(cmnt + " Yr ID            Jan" + "     Feb     Mar     Apr     May     Jun     Jul" + "     Aug     Sep     Oct     Nov     Dec     " + year_title);
                }

                @out.WriteLine(cmnt + "-e-b----------eb------eb------eb------eb------e" + "b------eb------eb------eb------eb------eb------e" + "b------eb------eb--------e");
            }
            else
            {
                // Daily output...
                @out.WriteLine(cmnt + "Yr  Mo ID            d(x,1)  d(x,2)  d(x,3)  " + "d(x,4)  d(x,5)  d(x,6)  d(x,7)  d(x,8)  d(x,9) " + "d(x,10) d(x,11) d(x,12) d(x,13) d(x,14) d(x,15) " + "d(x,16) d(x,17) d(x,18) d(x,19) d(x,20) d(x,21) " + "d(x,22) d(x,23) d(x,24) d(x,25) d(x,26) d(x,27) " + "d(x,28) d(x,29) d(x,30) d(x,31)   " + year_title);

                @out.WriteLine(cmnt + "--xx--xb----------eb------eb------eb------eb------e" + "b------eb------eb------eb------eb------eb------eb------e" + "b------eb------eb------eb------eb------eb------eb------e" + "b------eb------eb------eb------eb------eb------eb------e" + "b------eb------eb------eb------eb------eb------eb--------e");
            }
            ...
            if (standard_ts)
            {
                ...
                format = "   %2d/%4d  -     %2d/%4d  %5.5s" + string.Format(yeartype, "%5.5s");
            ...
            else
            {
                ...
                format = "   %2d/   0  -     %2d/   0%5.5s" + string.Format(yeartype, "%5.5s");
            ...
            if (req_interval_base == TimeInterval.MONTH)
            {
                iline_v = new List<object>();
                iline_format_v = new List<string>();
            }
            else
            {
                // Daily...
        ...
        public static void writeTimeSeriesList(IList<TS> tslist, PropList props)
        {
            ...
            if (!string.ReferenceEquals(prop_value, null))
            {
                precision = int.Parse(prop_value);
            }
            ...
            try
            {
                ...
                IList<string> ignoredCommentIndicators = new List<string>(1);
                ignoredCommentIndicators.Add("#>");
                //TODO @jurentie 04/05/2019 - fix header
                //@out = IOUtil.processFileHeaders(IOUtil.getPathUsingWorkingDir(infile), IOUtil.getPathUsingWorkingDir(outfile), newComments, commentIndicators, ignoredCommentIndicators, 0);
                @out = new StreamWriter(outfile);
                if (@out == null)
                {
