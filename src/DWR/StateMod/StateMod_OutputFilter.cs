...
namespace DWR.StateMod
{
	...
	/// passed.
	/// </summary>
	public class StateMod_OutputFilter : ProcessManagerOutputFilter
	{
		/// <summary>
		...
		/// <summary>
		/// Filter the string. </summary>
		/// <param name="line"> Line of output to check. </param>
		/// <returns> The string if it is a major progress indicator (new month and year)
		/// or null if not a major progress indicator. </returns>
		public virtual string filterOutput(string line)
		{
			if (line.StartsWith(" Execut;  Year", StringComparison.Ordinal))
			{
				// Output the line only if a new year and month.
				string part = line.Substring(0,30);
				if ((string.ReferenceEquals(__lastPrintedExecutLinePart, null)) || !part.Equals(__lastPrintedExecutLinePart))
				{
